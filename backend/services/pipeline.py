...
from core.config import ClassesNames, PersonStates
from core.decorators import safe_execute
from ml import ocr
from core.utils import class_count, check_motionless, check_PPE_intersections

if TYPE_CHECKING:
    from services.message import TextArea
    from visor_onnx_test.backend.ml.detection import DetectionModel
    from services.person import PersonTemplate
    from core.thread_storage import ThreadResult
    from vector_db.qdrant import QdrantRecognizer
    ...
    def _process_main_bbox(
        ...
        if check_motionless(bbox):
            threading.Thread(target=self.process_worker, 
                args=(function, self.original_frame, bbox, self.storage)).start()
